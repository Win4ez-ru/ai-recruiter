...
по локальному профилю. Резюме пользователь выберет на официальной странице HH
при завершении отклика. В структурированном событии `hh_resume_sync_rejected`
сохраняется `request_id`, который можно передать поддержке API HeadHunter.
